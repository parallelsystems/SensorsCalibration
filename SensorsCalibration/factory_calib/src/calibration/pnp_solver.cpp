...
/*
 * Cost function for solving LiDAR -> camera extrinsics
 *
 * `WCostFunctorPnP` predicts a rotation and translation between a set of points in 3D and a set in 2D.
 * It assumes the 2D points come from a camera with known extrinsics. Additionally, it is assumed that
 * the camera inputs have already been corrected for distortion. It predicts rotation in axis-angle form,
 * producing a vector r = [rx, ry, rz]. The rotation axis is r and the rotation angle is ||r|| (radians).
 */
struct WCostFunctorPnP
{
    std::vector<float> object_point_;
    std::vector<float> image_point_;
    std::vector< std::vector<double> > K_;
    std::vector<double> D_;
    WCostFunctorPnP( std::vector<float> objpoint,  std::vector<float> imgpoint,  std::vector< std::vector<double> > camera_intrinsic,  std::vector<double> dist_coeffs):
    ...
    static ceres::CostFunction* create( std::vector<float> objpoint, 
         std::vector<float> imgpoint,  std::vector< std::vector<double> > camera_intrinsic,  std::vector<double> dist_coeffs)
    {
        return new ceres::AutoDiffCostFunction<WCostFunctorPnP, 2, 3, 3>
            (new WCostFunctorPnP(objpoint, imgpoint, camera_intrinsic, dist_coeffs));
    }
};
...
bool solveCamPnP(std::vector< std::vector<float> >& objpoints,  std::vector< std::vector<float> >& imgpoints, 
    ...
    for (int i = 0; i < imgpoints.size(); i++)
    {
        ceres::CostFunction* cost = WCostFunctorPnP::create(objpoints[i], imgpoints[i], camera_intrinsic, dist_coeffs);
        problem.AddResidualBlock(cost, nullptr, rot, tra);
    }
